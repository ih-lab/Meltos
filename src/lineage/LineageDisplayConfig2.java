...
public class LineageDisplayConfig2 {
	...
	protected LineageTree spanningTree;
	protected PHYNetwork constrNetwork;
	protected HashMap<Integer, TreeNode> nodeInfo;
	protected ArrayList<String> sampleNames;
	protected Random randGen;
	
	
	public LineageDisplayConfig2(LineageTree tree, int numSamples, ArrayList<String> samples, boolean colorful) {
	...
	public void computeDisplayTree() {			
		...
		nodeInfo = new HashMap<Integer, TreeNode>();
		int edgeId = 0;
		for (Integer nI : spanningTree.edges.keySet()) {
			TreeNode n = spanningTree.nodes.get(nI);
			displayTree.addVertex(n.getNodeId());
			nodeInfo.put(n.getNodeId(), n);
			for(Integer nI2 : spanningTree.edges.get(nI)) {
				TreeNode n2 = spanningTree.nodes.get(nI2);
		...
		for(int i = 1; i < constrNetwork.numSamples; i++) {
			TreeNode n = new PHYNode(0, i, constrNetwork.numNodes + i);
			displayTree.addVertex(n.getNodeId());
			nodeInfo.put(n.getNodeId(), n);
	...
	public String getConsoleText(Integer nid) {
		...
		if(n.isLeaf()) { // sample 
    		...
    		s += "Binary sample presence profile: " + n.getSVGroup().getTag() + "\n";
    		int[] sampleIds = n.getSVGroup().getSampleIds();
    		s+= "Samples (in order):";
    		for(int i = 0; i < n.getSVGroup().getNumSamples(); i++) {
